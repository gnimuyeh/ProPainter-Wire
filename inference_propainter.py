...
primaries_dict = {
    ...
    6: Imath.Chromaticities(Imath.V2f(0.630, 0.340), Imath.V2f(0.310, 0.595), Imath.V2f(0.155, 0.070), Imath.V2f(0.3127, 0.3290)),
    5: Imath.Chromaticities(Imath.V2f(0.640, 0.330), Imath.V2f(0.290, 0.600), Imath.V2f(0.150, 0.060), Imath.V2f(0.3127, 0.3290)),
    7: Imath.Chromaticities(Imath.V2f(0.630, 0.340), Imath.V2f(0.310, 0.595), Imath.V2f(0.155, 0.070), Imath.V2f(0.3127, 0.3290)),
    8: Imath.Chromaticities(Imath.V2f(0.681, 0.319), Imath.V2f(0.243, 0.692), Imath.V2f(0.145, 0.049), Imath.V2f(0.3167, 0.3345)),
    10: Imath.Chromaticities(Imath.V2f(0.7347, 0.2653), Imath.V2f(0.0, 1.0), Imath.V2f(0.0001, -0.0770), Imath.V2f(0.32168, 0.33767)),
    11: Imath.Chromaticities(Imath.V2f(0.680, 0.320), Imath.V2f(0.265, 0.690), Imath.V2f(0.150, 0.060), Imath.V2f(0.314, 0.351)),
    12: Imath.Chromaticities(Imath.V2f(0.680, 0.320), Imath.V2f(0.265, 0.690), Imath.V2f(0.150, 0.060), Imath.V2f(0.3127, 0.3290)),
    22: Imath.Chromaticities(Imath.V2f(0.630, 0.340), Imath.V2f(0.295, 0.605), Imath.V2f(0.155, 0.077), Imath.V2f(0.3127, 0.3290)),
    4: Imath.Chromaticities(Imath.V2f(0.67, 0.33), Imath.V2f(0.21, 0.71), Imath.V2f(0.14, 0.08), Imath.V2f(0.310, 0.316)),
}
...
# --- Helper Functions (Interface Preserved) ---
...
def imwrite(img, file_path, params=None, auto_mkdir=True, color_info=None):
    ...
    if color_info and color_info['primaries'] is not None:
        ...
        header['chromaticities'] = primaries_dict[1]
...
def save_video_highest_quality(frames, output_path, fps, color_info=None):
    ...
    cmd = [
        'ffmpeg', '-y', '-f', 'rawvideo', '-pix_fmt', 'rgb48le', '-s', f'{w}x{h}', '-r', str(fps),
        '-i', '-', '-c:v', 'prores_ks', '-profile:v', '5', '-pix_fmt', 'yuv444p12le',
        '-vendor', 'apl0', '-bits_per_mb', '8000', '-quant_mat', 'hq', '-movflags', '+write_colr+faststart', '-write_tmcd', '0',
        output_path
    ]
    ...
    cmd.insert(-1, '-color_primaries'); cmd.insert(-1, primaries_name)
    cmd.insert(-1, '-colorspace'); cmd.insert(-1, matrix_name)
    cmd.insert(-1, '-color_trc'); cmd.insert(-1, transfer_name)
    cmd.insert(-1, '-color_range'); cmd.insert(-1, range_name)
...
def pad_tensor_to_modulo(img_tensor, mod=16):
    """
    Pads a 5D tensor (B, T, C, H, W) to be divisible by mod.
    Uses 'replicate' padding by flattening to 4D first.
    """
    b, t, c, h, w = img_tensor.shape
    pad_h = (mod - h % mod) % mod
    pad_w = (mod - w % mod) % mod
    if pad_h > 0 or pad_w > 0:
        flat_tensor = img_tensor.view(b * t, c, h, w)
        padded_flat = F.pad(flat_tensor, (0, pad_w, 0, pad_h), mode='replicate')
        return padded_flat.view(b, t, c, h + pad_h, w + pad_w), (h, w)
...
def read_frame_from_videos(frame_root):
    ...
    if frame_root.endswith(('mp4', 'mov', 'avi', 'MP4', 'MOV', 'AVI')):
        ...
        if color_info['primaries'] is None: color_info['primaries'] = 1
        if color_info['range'] is None or color_info['range'] == 0: video_stream.codec_context.color_range = 1; color_info['range'] = 1
        frames = []
        frames_pil = []
...
# --- FIX: Restored Original Signature for load_models ---
def load_models(device):
    ...
    # --- Optimization: Compile (Safe to run internally) ---
    try:
        print("Compiling ProPainter model for efficiency...")
        model = torch.compile(model, mode="reduce-overhead")
    except Exception as e:
        print(f"Compilation warning (safe to ignore if on old PyTorch): {e}")
...
def run_inference(video, mask, output='results', resize_ratio=1.0, height=-1, width=-1, mask_dilation=0,
    ...
    # FIX: Handle model loading inside here if not provided
    if models is None:
        models = load_models(device)
    
    # FIX: Apply half precision logic HERE, not in load_models
    ...
    # Convert Inpainter to half if requested (RAFT/Flow kept in FP32 for stability)
    if use_half:
        model = model.half()
    ...
    if (width != -1 and height != -1) or resize_ratio != 1.0:
    ...
    frames, _ = pad_tensor_to_modulo(frames, mod=16)
    pad_h = frames.shape[-2] - original_h
    pad_w = frames.shape[-1] - original_w
    
    if pad_h > 0 or pad_w > 0:
        flow_masks = F.pad(flow_masks, (0, pad_w, 0, pad_h), mode='constant', value=0)
        masks_dilated = F.pad(masks_dilated, (0, pad_w, 0, pad_h), mode='constant', value=0)
    
    b, t, c, h, w = frames.shape 
    video_length = frames.size(1)
    print(f'\nProcessing: {video_name} [{video_length} frames] [Res: {w}x{h}]...')
    
    with torch.backends.cuda.sdp_kernel(enable_flash=True, enable_math=True, enable_mem_efficient=True):
        with torch.no_grad():
            with torch.inference_mode():
                # --- 1. RAFT Flow ---
                if frames.size(-1) <= 640: short_clip_len = 12
                elif frames.size(-1) <= 720: short_clip_len = 8
                elif frames.size(-1) <= 1280: short_clip_len = 4
                ...
                for f in range(0, video_length, short_clip_len):
                    ...
                    if f == 0: flows_f, flows_b = fix_raft(frames[:, f:end_f], iters=raft_iter)
                    else: flows_f, flows_b = fix_raft(frames[:, f-1:end_f], iters=raft_iter)
                ...
                for f in range(0, flow_length, subvideo_length):
                    ...
                    curr_gt_f = gt_flows_f[:, s_f:e_f].to(device)
                    curr_gt_b = gt_flows_b[:, s_f:e_f].to(device)
                    curr_masks = flow_masks[:, s_f:e_f+1].to(device)
                    
                    pred_flows_bi_sub, _ = fix_flow_complete.forward_bidirect_flow(
                        (curr_gt_f, curr_gt_b), curr_masks)
                    
                    pred_flows_bi_sub = fix_flow_complete.combine_flow(
                        (curr_gt_f, curr_gt_b), pred_flows_bi_sub, curr_masks)
                    ...
                    del pred_flows_bi_sub, curr_gt_f, curr_gt_b, curr_masks
                    torch.cuda.empty_cache()
                    
                del gt_flows_f, gt_flows_b
                gc.collect()
                ...
                if use_half:
                    frames_inf = frames.half()
                    masks_dilated_inf = masks_dilated.half()
                else:
                    frames_inf = frames
                    masks_dilated_inf = masks_dilated

                masked_frames = frames_inf * (1 - masks_dilated_inf)
                ...
                with autocast(enabled=use_half):
                    ...
                    for f in range(0, video_length, subvideo_length_img_prop):
                        ...
                        b_sz, t_sz, _, _, _ = masks_dilated_inf[:, s_f:e_f].size()
                        
                        curr_flows = (pred_flows_f[:, s_f:e_f-1].to(device), pred_flows_b[:, s_f:e_f-1].to(device))
                        if use_half:
                            curr_flows = (curr_flows[0].half(), curr_flows[1].half())

                        prop_imgs_sub, updated_local_masks_sub = model.img_propagation(
                            masked_frames[:, s_f:e_f],
                            curr_flows,
                            masks_dilated_inf[:, s_f:e_f],
                            'nearest'
                        )
                        
                        updated_frames_sub = frames_inf[:, s_f:e_f] * (1 - masks_dilated_inf[:, s_f:e_f]) + \
                                            prop_imgs_sub.view(b_sz, t_sz, 3, h, w) * masks_dilated_inf[:, s_f:e_f]
                        updated_masks_sub = updated_local_masks_sub.view(b_sz, t_sz, 1, h, w)
                        
                        updated_frames_cpu.append(updated_frames_sub[:, pad_len_s:e_f-s_f-pad_len_e].cpu())
                        updated_masks_cpu.append(updated_masks_sub[:, pad_len_s:e_f-s_f-pad_len_e].cpu())
                        
                        del curr_flows, prop_imgs_sub, updated_frames_sub, updated_masks_sub, updated_local_masks_sub
                        torch.cuda.empty_cache()
                        gc.collect()

                updated_frames = torch.cat(updated_frames_cpu, dim=1)
                updated_masks = torch.cat(updated_masks_cpu, dim=1)
                del updated_frames_cpu, updated_masks_cpu, masked_frames
                gc.collect()

                # --- 4. Final Transformer Refinement ---
                ...
                for f in tqdm(range(0, video_length, neighbor_stride)):
                    ...
                    indices = neighbor_ids + ref_ids
                    
                    selected_imgs = updated_frames[:, indices].to(device)
                    selected_masks = masks_dilated_inf[:, indices].to(device)
                    selected_update_masks = updated_masks[:, indices].to(device)
                    
                    flow_indices = neighbor_ids[:-1]
                    s_flow_f = pred_flows_f[:, flow_indices].to(device)
                    s_flow_b = pred_flows_b[:, flow_indices].to(device)
                    
                    if use_half:
                        selected_imgs = selected_imgs.half()
                        selected_update_masks = selected_update_masks.half()
                        s_flow_f = s_flow_f.half()
                        s_flow_b = s_flow_b.half()

                    selected_pred_flows_bi = (s_flow_f, s_flow_b)
                    
                    with autocast(enabled=use_half):
                        l_t = len(neighbor_ids)
                        pred_img = model(selected_imgs, selected_pred_flows_bi, selected_masks, selected_update_masks, l_t)
                        
                        pred_img = pred_img[:, :, :original_h, :original_w]
                        pred_img = pred_img.view(-1, 3, original_h, original_w)
                        pred_img = (pred_img + 1) / 2
                        pred_img = pred_img.float().cpu().permute(0, 2, 3, 1).numpy()
                    
                    binary_masks = masks_dilated[0, neighbor_ids, :, :original_h, :original_w].cpu().permute(0, 2, 3, 1).numpy().astype(np.float32)
                    
                    for i in range(len(neighbor_ids)):
                        idx = neighbor_ids[i]
                        ori_scaled = ori_frames[idx].astype(np.float32) / 65535.0
                    ...
                    del selected_imgs, selected_masks, selected_update_masks, selected_pred_flows_bi, pred_img, binary_masks, s_flow_f, s_flow_b
                    torch.cuda.empty_cache()
                    gc.collect()
    ...
    if save_frames:
        ...
        if save_masked_in and masked_frame_for_save:
            if masked_frame_for_save[0].shape[:2] != (out_size[1], out_size[0]):
