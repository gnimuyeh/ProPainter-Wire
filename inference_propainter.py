...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize_ratio', type=float, default=0.7, help='Resize scale for processing video.')
    ...
    parser.add_argument('--mask_dilation', type=int, default=2, help='Mask dilation for video and flow masking.')
    parser.add_argument('--ref_stride', type=int, default=20, help='Stride of global reference frames.')
    parser.add_argument('--neighbor_length', type=int, default=18, help='Length of local neighboring frames.')
    ...
    original_size = size  # Store original dimensions
    ...
    if args.mode == 'video_inpainting':
        ...
        flow_masks, masks_dilated = read_mask(args.mask, frames_len, size, flow_mask_dilates=args.mask_dilation, mask_dilates=args.mask_dilation)
        w, h = size
    ...
    ckpt_path = load_file_from_url(url=os.path.join(pretrain_model_url, 'raft-things.pth'), model_dir='weights', progress=True, file_name=None)
    fix_raft = RAFT_bi(ckpt_path, device)
    ckpt_path = load_file_from_url(url=os.path.join(pretrain_model_url, 'recurrent_flow_completion.pth'), model_dir='weights', progress=True, file_name=None)
    fix_flow_complete = RecurrentFlowCompleteNet(ckpt_path)
    for p in fix_flow_complete.parameters():
        p.requires_grad = False
    fix_flow_complete.to(device)
    fix_flow_complete.eval()
    ckpt_path = load_file_from_url(url=os.path.join(pretrain_model_url, 'ProPainter.pth'), model_dir='weights', progress=True, file_name=None)
        ...
        if flow_length > args.subvideo_length:
            ...
            for f in range(0, flow_length, args.subvideo_length):
                ...
                pred_flows_bi_sub = fix_flow_complete.combine_flow(
                    (gt_flows_bi[0][:, s_f:e_f], gt_flows_bi[1][:, s_f:e_f]), pred_flows_bi_sub, flow_masks[:, s_f:e_f+1])
    ...
    # Post-processing: Resize and blend using Poisson blending
    if args.save_frames:
        blended_frames = []
        for idx in range(video_length):
            # Resize inpainted frame and mask to original size
            inpainted = cv2.resize(comp_frames[idx], original_size, interpolation=cv2.INTER_CUBIC)
            mask = cv2.resize(np.array(masks_dilated[0, idx].cpu().numpy()[0] * 255, dtype=np.uint8),
                              original_size, interpolation=cv2.INTER_NEAREST)
            orig = ori_frames[idx]
            # Apply Poisson blending
            center = (original_size[0] // 2, original_size[1] // 2)
            blended = cv2.seamlessClone(inpainted, orig, mask, center, cv2.NORMAL_CLONE)
            blended_frames.append(blended)
            # Save individual frame
            img_save_root = os.path.join(save_root, 'frames', f"{idx:04d}.png")
            imwrite(cv2.cvtColor(blended, cv2.COLOR_RGB2BGR), img_save_root)
    else:
        blended_frames = comp_frames

    masked_frame_for_save = [cv2.resize(f, original_size) for f in masked_frame_for_save]
    blended_frames = [cv2.resize(f, original_size) for f in blended_frames if not args.save_frames]
    imageio.mimwrite(os.path.join(save_root, 'masked_in.mp4'), masked_frame_for_save, fps=fps, quality=7)
    imageio.mimwrite(os.path.join(save_root, 'inpaint_out.mp4'), blended_frames, fps=fps, quality=7)
